...
const VisitPage = {
    ...
    getFormData() {
        ...
        const visitTarget = getValue('visitTarget');

        let targetPerson = '';
        let targetPhone = '';

        if (visitTarget === 'teacher') {
            const teacherSelect = document.getElementById('targetTeacher');
            if (teacherSelect) {
                const selectedOption = teacherSelect.options[teacherSelect.selectedIndex];
                targetPerson = selectedOption ? selectedOption.text : '';
            }
        } else if (visitTarget === 'custom') {
            targetPerson = getValue('customTargetName');
            targetPhone = getValue('customTargetPhone');
        }

        return {
            visit_date: getValue('visitDate'),
            time_start: getValue('timeStart'),
            time_end: getValue('timeEnd'),
    ...
    validateForm() {
        ...
        const visitTarget = getValue('visitTarget');
        if (visitTarget === 'teacher') {
            if (!getValue('targetDepartment')) errors.push('请选择拜访部门');
            if (!getValue('targetTeacher')) errors.push('请选择拜访教师');
        } else if (visitTarget === 'custom') {
            if (!getValue('customTargetName')) errors.push('请填写拜访对象姓名');
            if (!getValue('customTargetPhone')) errors.push('请填写拜访对象电话');
        } else {
            errors.push('请选择访问对象');
        }
        ...
        const targetPhone = getValue('customTargetPhone');
        if (visitTarget === 'custom' && targetPhone && !Utils.validatePhone(targetPhone)) {
            errors.push('拜访对象电话格式不正确');
        }
        ...
        return `
            <div class="history-item" data-id="${app.id}">
                ...
                <div class="history-content">
                    ...
                    <div class="history-time">时间: ${Utils.formatTime(app.time_start)} - ${Utils.formatTime(app.time_end)}</div>
                </div>
...
const FacePage = {
    ...
    async startFaceRegistration() {
        try {
            ...
            // 启动人脸注册流程
            await this.captureFaceForRegistration();
        } catch (error) {
            Utils.hideLoading();
    ...
    async captureFaceForRegistration() {
        try {
            ...
            Utils.hideLoading();
            Utils.showToast('人脸注册失败: ' + error.message, 'error');
        }
    },
            ...
            <div class="record-item">
                ...
                <div class="record-content">
                    ...
                    <div class="record-time">
                        ${Utils.formatTime(record.time_start)} - ${Utils.formatTime(record.time_end)}
                    </div>
            ...
            <div class="application-item" data-id="${app.id}">
                ...
                <div class="application-content">
                    ...
                    <div class="application-details">
                        ...
                        <div><strong>访问时间:</strong> ${Utils.formatTime(app.time_start)} - ${Utils.formatTime(app.time_end)}</div>
            ...
            const detailsContent = `
                <div class="application-details">
                    ...
                    <div class="detail-section">
                        ...
                        <p><strong>访问时间:</strong> ${Utils.formatTime(app.time_start)} - ${Utils.formatTime(app.time_end)}</p>
