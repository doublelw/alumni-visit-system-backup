...
const AlumniApprovePage = {
    ...
    renderAlumniTable(alumni) {
        ...
        if (alumni.length === 0) {
            tbody.innerHTML = '<tr><td colspan="13" class="text-center text-secondary">暂无数据</td></tr>';
            return;
        }

        tbody.innerHTML = alumni.map((alumni, index) => {
...
const PaginationHelper = {
    ...
    onPerPageChange(containerId, newPerPage, loadFunction) {
        ...
        if (pageName && window[pageName]) {
            ...
            // 特殊处理AlumniApprovePage
            if (pageName === 'AlumniApprovePage') {
                window[pageName].loadAlumni();
                return;
            }
    ...
    getPageNameFromContainer(containerId) {
        const mapping = {
            ...
            'alumniPagination': 'AlumniApprovePage',
