...
def test_user_improvements():
    ...
    try:
        ...
        if create_response.status_code == 200:
            ...
            print(f"❌ 创建教师用户失败: {create_response.status_code}")
        ...
        if create_response2.status_code == 200:
            ...
            if multi_user['is_visitable']:
                print("✅ 包含教师身份的用户自动设置为可拜访")
            else:
                print("❌ 包含教师身份的用户未自动设置为可拜访")
        else:
            print(f"❌ 创建多身份用户失败: {create_response2.status_code}")
        ...
        if create_response3.status_code == 200:
            ...
            if not student_user['is_visitable']:
                ...
                print("❌ 非教师用户被错误设置为可拜访")
        else:
            print(f"❌ 创建学生用户失败: {create_response3.status_code}")
