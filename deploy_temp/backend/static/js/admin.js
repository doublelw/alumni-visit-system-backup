...
const PaginationHelper = {
    ...
    // 渲染带每页条数选择的分页
    renderPaginationWithPerPage(pagination, containerId, callbacks) {
        ...
        html += '<select id="perPageSelect" class="form-select" onchange="' + callbacks.changePerPage + '(this.value)">';
        this.perPageOptions.forEach(option => {
            html += `<option value="${option}" ${pagination.per_page == option ? 'selected' : ''}>${option} 条/页</option>`;
        });
        html += '</select>';
        html += '</div>';
        ...
        // 上一页
        html += `<button ${pagination.has_prev ? '' : 'disabled'} onclick="${callbacks.goToPage}(${pagination.page - 1})">上一页</button>`;
        ...
        // 第一页
        if (startPage > 1) {
            html += `<button onclick="${callbacks.goToPage}(1)">1</button>`;
            if (startPage > 2) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            html += `<button class="${i === pagination.page ? 'active' : ''}" onclick="${callbacks.goToPage}(${i})">${i}</button>`;
        }
        ...
        if (endPage < pagination.pages) {
            ...
            html += `<button onclick="${callbacks.goToPage}(${pagination.pages})">${pagination.pages}</button>`;
        }

        // 下一页
        html += `<button ${pagination.has_next ? '' : 'disabled'} onclick="${callbacks.goToPage}(${pagination.page + 1})">下一页</button>`;

        html += '</div>';

        // 页面信息
        html += '<div class="pagination-info">';
        html += `共 ${pagination.total} 条记录，第 ${pagination.page} 页，共 ${pagination.pages} 页`;
        html += '</div>';
...
const AlumniApprovePage = {
    ...
    async loadPendingAlumni() {
        try {
            const data = await AdminUtils.request(`/admin/alumni-approve?page=${this.currentPage}&per_page=${this.perPage}`);
    ...
    renderPagination(pagination) {
        PaginationHelper.renderPaginationWithPerPage(
            pagination,
            'alumniApprovePagination',
            {
                goToPage: 'AlumniApprovePage.goToPage',
                changePerPage: 'AlumniApprovePage.changePerPage'
            }
        );
    },
