...
def register():
    ...
    try:
        ...
        current_app.logger.info(f"用户 {user.username} 活动报名成功，验证码: {verification_code}")
        ...
        return jsonify({
            ...
            'verification_code': verification_code,
            'redirect_url': f'/verification-code?code={verification_code}'
        }), 201
...
@event_bp.route('/debug-registrations', methods=['GET'])
def debug_registrations():
    ...
    try:
        ...
        # 直接查询数据库
        result = db.session.execute('SELECT id, real_name, verification_code FROM event_registrations ORDER BY id')
        ...
        # 使用SQLAlchemy ORM查询
        orm_regs = EventRegistration.query.all()
        orm_info = []
        for reg in orm_regs:
            orm_info.append({
                'id': reg.id,
                ...
                'verification_code_type': type(reg.verification_code)
            })

        return jsonify({
            'success': True,
            'raw_sql_results': debug_info,
            'orm_results': orm_info
        })
    ...
    try:
        ...
        for reg in registrations.items:
            registration_list.append({
                ...
                'real_name': reg.real_name,
                'will_dine': reg.will_dine,
                'favorite_dishes': reg.favorite_dishes,
                'registration_time': reg.registration_time.strftime('%Y-%m-%d %H:%M:%S') if reg.registration_time else None,
