...
def get_users():
    ...
    try:
        ...
        # 关联校友档案搜索
        if search:
            query = query.outerjoin(AlumniProfile, User.id == AlumniProfile.user_id).filter(
...
def delete_user(user_id):
    ...
    try:
        ...
        visit_records = VisitRecord.query.filter_by(user_id=user_id).all()
        for record in visit_records:
            db.session.delete(record)

        # 删除相关的访问申请
        from app.models import VisitApplication
        visit_applications = VisitApplication.query.filter_by(applicant_id=user_id).all()
        for app in visit_applications:
            db.session.delete(app)

        # 删除相关的活动注册记录（使用原生SQL避免关系问题）
        db.session.execute(text("DELETE FROM event_registrations WHERE user_id = :user_id"), {'user_id': user_id})
        ...
        # 删除相关的人脸历史记录
        from app.models import FaceHistory
        face_histories = FaceHistory.query.filter_by(user_id=user_id).all()
        for history in face_histories:
            db.session.delete(history)

        # 删除相关的学生离校申请
        from app.models import StudentExitApplication
        exit_applications = StudentExitApplication.query.filter_by(
            (StudentExitApplication.student_id == user_id) |
            (StudentExitApplication.applicant_id == user_id)
        ).all()
        ...
        if user_record:
            # 如果是家长，清除与学生的关联
            if user_record.user_type == 'parent':
                user_record.parent_students = []
            # 如果是学生，清除与家长的关联
            elif user_record.user_type == 'student':
                user_record.student_parents = []
        ...
        return jsonify({'error': error_msg, 'detail': str(e)}), 500
...
def import_users_preview():
    ...
    try:
        ...
        if not file.filename.lower().endswith(('.xlsx', '.xls', '.csv')):
            return jsonify({'error': '请上传Excel文件 (.xlsx, .xls) 或 CSV文件 (.csv)'}), 400

        # 读取文件（支持Excel和CSV）
        try:
            if file_type == 'csv' or file.filename.lower().endswith('.csv'):
                # 读取CSV文件，尝试不同的编码
                try:
        ...
        # 字段映射配置（支持制卡中心格式和标准格式）
        field_mappings = {
            # 制卡中心格式（新增，优先）
            'username': ['学（工）号', '学号', '工号', 'username', '用户名', 'Username'],
            'real_name': ['客户姓名', '姓名', '真实姓名', 'Name', '姓名'],
            'user_type': ['客户身份', '用户类型', 'User Type', '类型'],
            'id_card': ['证件号', '身份证号', 'ID Card'],
            'gender': ['性别（1为男0为女）', '性别', 'Gender'],
            'phone': ['手机号', '联系电话', 'Phone', 'Tel', '电话'],
            'subsidy_type': ['补助类型（0～29）', '补助类型'],
            'org_level1': ['一级部门', '一级组织'],
            'org_level2': ['二级部门', '二级组织'],
            'org_level3': ['三级部门', '三级组织'],
            'org_level4': ['四级部门', '四级组织'],
            # 标准格式（保留）
            'email': ['邮箱', 'Email', '电子邮箱'],
            'student_id': ['学号', 'Student ID'],
            'employee_id': ['工号', 'Employee ID'],
            'class_id': ['class_id', '班级', 'Class'],
            'grade': ['grade', '年级', 'Grade'],
            'notes': ['notes', '备注', 'Remarks', 'Note']
        }
        ...
        # 检查必需的列（支持制卡中心格式）
        required_columns = ['username', 'real_name']
        ...
        for index, row in df.iterrows():
            # 获取用户类型，默认为校友
            user_type_raw = str(row.get('user_type', '校友')).strip() if pd.notna(row.get('user_type')) else '校友'
            user_type = user_type_mapping.get(user_type_raw, 'alumni')

            # 处理用户名（学号/工号）
            username = str(row.get('username', '')).strip()
            if not username:
                validation_errors = ['用户名（学/工号）不能为空']
                users.append({
                    'username': '',
            ...
            # 如果CSV中没有邮箱，使用用户名@domain.com作为临时邮箱
            email = str(row.get('email', '')).strip() if pd.notna(row.get('email')) else ''
            if not email:
                email = f"{username}@temp.alumni.edu"
            ...
            if user_type == 'student' and not student_id:
                student_id = username  # 学生使用用户名作为学号
            if user_type == 'teacher' and not employee_id:
                employee_id = username  # 教师使用用户名作为工号
            ...
            user_data = {
                ...
                'class_id': str(row.get('class_id', '')).strip() if pd.notna(row.get('class_id')) else '',
                'grade': str(row.get('grade', '')).strip() if pd.notna(row.get('grade')) else '',
                'id_card': str(row.get('id_card', '')).strip() if pd.notna(row.get('id_card')) else '',
                'gender': 'male' if str(row.get('gender', '')).strip() == '1' else 'female' if pd.notna(row.get('gender')) else '',
                'subsidy_type': int(row.get('subsidy_type', 0)) if pd.notna(row.get('subsidy_type')) else 0,
                'org_level1': str(row.get('org_level1', '')).strip() if pd.notna(row.get('org_level1')) else '',
                'org_level2': str(row.get('org_level2', '')).strip() if pd.notna(row.get('org_level2')) else '',
                'org_level3': str(row.get('org_level3', '')).strip() if pd.notna(row.get('org_level3')) else '',
                'org_level4': str(row.get('org_level4', '')).strip() if pd.notna(row.get('org_level4')) else '',
            ...
            if not user_data['username']:
                validation_errors.append('用户名（学/工号）不能为空')
                is_valid = False
            ...
            # 验证邮箱格式（但如果邮箱是临时生成的，则不验证）
            import re
            if user_data['email'] and not user_data['email'].endswith('@temp.alumni.edu'):
        ...
        for user_data in users:
            try:
                ...
                new_user = User(
                    ...
                    # 制卡中心新增字段
                    id_card=user_data.get('id_card') or None,
                    # 性别：1为男，0为女
                    is_class_teacher=False,  # 默认不是班主任
                    # 组织层级信息（暂时存储在notes字段）
                    notes=f"组织: {user_data.get('org_level1', '')} > {user_data.get('org_level2', '')} > {user_data.get('org_level3', '')} > {user_data.get('org_level4', '')} | 补助类型: {user_data.get('subsidy_type', 0)}" if user_data.get('org_level1') else user_data.get('notes', ''),
                    status='active',
...
def download_user_template():
    ...
    try:
        ...
        # 创建模板数据 - 按照制卡中心数据导入模板的格式
        template_data = {
            '学（工）号': ['S2023001', 'T2023001', 'A2023001'],
            '客户姓名': ['张三', '李四', '王五'],
            '一级部门': ['教务处', '计算机学院', '文学院'],
            '二级部门': ['教学管理科', '软件工程系', '中文系'],
            '三级部门': ['本科教学', [''], ['']],
            '四级部门': [[''], [''], ['']],
            '补助类型（0～29）': [1, 5, 10],
            '客户身份': ['teacher', 'teacher', 'alumni'],
            '证件号': ['110101199001011234', '110101199001011235', '110101199001011236'],
            '性别（1为男0为女）': [1, 0, 1],
        ...
        # 创建Excel文件 - 使用.xls格式以兼容旧版本
        output = BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='制卡数据', index=False)
        ...
        return current_app.response_class(
            ...
            headers={'Content-Disposition': 'attachment; filename=制卡中心数据导入模板.xlsx'}
        )
