...
def generate_math_captcha():
    ...
    draw.text((10, 15), question, fill=(0, 0, 0), font=font)
    ...
    session.permanent = False  # 确保session在浏览器关闭时过期

    return {
        'image': image_base64,
        'answer': str(answer)  # 调试用，生产环境中应该移除
    }
...
def register():
    ...
    try:
        ...
        # 验证基本信息必填字段（邮箱改为选填）
        basic_fields = ['username', 'password', 'realName', 'phone']
        ...
        # 验证工作信息必填字段
        work_fields = ['currentCity', 'workUnit']
        for field in work_fields:
            if not data.get(field) or data.get(field).strip() == '':
                return jsonify({'success': False, 'message': f'{field}不能为空'}), 400

        # 验证就餐安排信息（可选）
        dining_count = 1  # 默认1人就餐
        if data.get('diningCompanions') and data.get('diningCompanions').strip():
            try:
                dining_count = int(data['diningCompanions'])
                if dining_count < 1 or dining_count > 20:
                    return jsonify({'success': False, 'message': '就餐人数应在1-20人之间'}), 400
            except ValueError:
                return jsonify({'success': False, 'message': '就餐人数格式不正确'}), 400
        ...
        if existing_user:
            ...
            # 添加数字后缀
            for i in range(1, 10):
                suggested_username = f"{base_username}{i}"
                if not User.query.filter_by(username=suggested_username).first():
            ...
            for i in range(3):
                ...
                suggested_username = f"{base_username}_{random_suffix}"
                if not User.query.filter_by(username=suggested_username).first():
                    suggestions.append(suggested_username)
        ...
        if User.query.filter_by(phone=data['phone']).first():
            return jsonify({
                ...
                'message': '该手机号已经注册',
                'field': 'phone',
                'detail': '一个手机号只能注册一个账户，请使用其他手机号进行注册'
            }), 400
        ...
        user = User(
            ...
            email=data.get('email', None),  # 邮箱改为可选
            phone=data['phone'],
            wechat=data.get('wechat', None),  # 添加微信号字段
            user_type='alumni',
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    current_app.logger.warning(f"数据库flush失败，重试 {attempt + 1}/{max_retries}: {str(flush_error)}")
                    db.session.rollback()
        ...
        # 处理身份证字段 - 如果为空，生成唯一值避免数据库UNIQUE约束冲突
        id_card_value = data.get('idCard', '').strip()
        if not id_card_value:
            # 使用UUID确保唯一性，避免并发时的冲突
            id_card_value = f"TMP{uuid.uuid4().hex[:16].upper()}"

        # 创建校友档案
        alumni_profile = AlumniProfile(
            user_id=user.id,
            student_id=data.get('studentId', f"AUTO{uuid.uuid4().hex[:8].upper()}"),  # 生成默认学号
            ...
            # 就餐安排（使用默认值1人）
            dining_companions=int(data.get('diningCompanions', 1)),
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    current_app.logger.warning(f"数据库commit失败，重试 {attempt + 1}/{max_retries}: {str(commit_error)}")
                    db.session.rollback()
                    time.sleep(0.1 * (attempt + 1))  # 递增延迟
                    db.session.add(user)
                    db.session.flush()  # 重新获取用户ID
                    db.session.add(alumni_profile)
                else:
                    raise commit_error
        ...
        if 'unique' in error_msg and 'id_card' in error_msg:
            return jsonify({'success': False, 'message': '身份证号已被使用，请检查后重新输入'}), 400
        elif 'unique' in error_msg and ('username' in error_msg or 'phone' in error_msg):
            return jsonify({'success': False, 'message': '用户名或手机号已被注册'}), 400
        elif 'foreign key' in error_msg:
            return jsonify({'success': False, 'message': '数据关联错误，请联系管理员'}), 500
        else:
...
def login():
    ...
    try:
        ...
        # 验证码验证 - 临时禁用用于测试登录问题
        current_app.logger.info("临时跳过验证码验证用于测试")
        stored_answer = session.get('captcha_answer')
        provided_captcha = data.get('captcha', '')
        current_app.logger.info(f"验证码调试 - Session验证码: '{stored_answer}', 用户输入: '{provided_captcha}' (已跳过验证)")
