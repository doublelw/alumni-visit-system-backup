    ...
    initForms() {
        ...
        document.getElementById('registerForm').addEventListener('submit', async (e) => {
            ...
            const errors = Utils.validateForm(formData, {
                ...
                password: { required: true, label: '密码', minLength: 8 },
                confirm_password: { required: true, label: '确认密码', confirm: 'password' }
            });
...
const VisitPage = {
    ...
    initCompanionInfo() {
        ...
        // 初始化一个同行人输入框
        if (document.querySelectorAll('.companion-item').length === 0) {
            this.addCompanion();
        }
    },
    ...
    addCompanion() {
        ...
        const companionCount = companionList.children.length;
        ...
        companionItem.innerHTML = `
            <div class="companion-header">
                ...
                <button type="button" class="btn btn-sm btn-outline-danger remove-companion"
                        title="删除同行人" ${companionCount === 0 ? 'style="display:none;"' : ''}>
    ...
    removeCompanion(companionItem) {
        const companionList = document.getElementById('companionList');
        if (!companionList || companionList.children.length <= 1) {
            Utils.showToast('至少保留一个同行人信息', 'warning');
            return;
        }
    ...
    updateRemoveButtons() {
        ...
        companionItems.forEach((item, index) => {
            ...
            if (removeBtn) {
                removeBtn.style.display = companionItems.length > 1 ? 'block' : 'none';
            }
        });
    },
    ...
    validateForm() {
        ...
        if (hasVehicle) {
            ...
            if (!licensePlate || !vehicleType || !vehicleColor) {
                Utils.showToast('请填写完整的车辆信息', 'warning');
                return false;
            }
        }
